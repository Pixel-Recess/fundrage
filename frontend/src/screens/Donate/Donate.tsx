...
/**
 * Built from Figma's "Donate" frame (31:1438). This screen only ever collects an amount —
 * no payment fields, no card entry — because in the real app, Next hands off to an Every.org
 * Donate Link (or Apple Pay via their flow); the actual charge happens entirely outside
 * FundRage, and confirmation comes back later via Every.org's webhook (CLAUDE.md §1). Building
 * an in-app "enter your card" step here would be a pattern that can't be replicated once this
 * is wired to a real backend, so this demo skips straight from Next to a mocked success screen
 * instead (see DonationSuccess) rather than inventing one.
 */
    ...
    <div className={styles.screen}>
      ...
      <div className={styles.content}>
        ...
        <label className={styles.field}>
          <span className={styles.fieldLabel}>Enter another amount</span>
        ...
        <p className={styles.sectionLabel}>More Details</p>
      ...
      <NavFooter onBack={onBack} onNext={() => onNext(amount)} nextEnabled={amount > 0} />
    </div>
  );
}
